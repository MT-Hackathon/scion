...
token="${GITHUB_PERSONAL_ACCESS_TOKEN:?GITHUB_PERSONAL_ACCESS_TOKEN is required}"
...
post_issue() {
  ...
  local body="$2"
  echo "Creating: $title"
  jq -n --arg title "$title" --arg body "$body" --argjson labels "$labels" '{title:$title, body:$body, labels:$labels}' \
    | curl -sS -X POST \
...
post_issue "Settings API: write to default_app_settings.json and en.json" "$(cat <<'EOB'
## Problem
...
post_issue "Settings API: rebuild trigger (Apply Changes support)" "$(cat <<'EOB'
## Problem
...
post_issue "Remove CopySettings duplication; migrate copy to en.json" "$(cat <<'EOB'
## Problem
...
post_issue "Settings UI: Add Apply Changes button (write + rebuild)" "$(cat <<'EOB'
## Problem
...
post_issue "Settings Navigation: reorganize sections for growth" "$(cat <<'EOB'
## Problem
...
post_issue "Settings UI: Typography editors" "$(cat <<'EOB'
## Problem
...
post_issue "Settings UI: Form layout editors" "$(cat <<'EOB'
## Problem
...
post_issue "Settings UI: Hotkey configuration" "$(cat <<'EOB'
## Problem
...
post_issue "Settings UI: Badge color token editors" "$(cat <<'EOB'
## Problem
...
post_issue "Settings UI: Canvas empty state styling" "$(cat <<'EOB'
## Problem
...
post_issue "Settings UI: Button variant editors" "$(cat <<'EOB'
## Problem
...
post_issue "Settings UI: Elevation/Shadow editors" "$(cat <<'EOB'
## Problem
...
post_issue "Node settings: add form-specific overrides" "$(cat <<'EOB'
## Problem
...
post_issue "Copy: add main page strings to en.json" "$(cat <<'EOB'
## Problem
...
post_issue "Copy: add toast messages to en.json" "$(cat <<'EOB'
## Problem
...
post_issue "Copy: add connection test messages to en.json" "$(cat <<'EOB'
## Problem
...
post_issue "Copy: add missing dialog copy to en.json" "$(cat <<'EOB'
## Problem
...
post_issue "Copy: canvas empty state text editors" "$(cat <<'EOB'
## Problem
...
post_issue "Docs: CSS override pattern for context styling" "$(cat <<'EOB'
## Problem
...
post_issue "Refactor: settings section renderer pattern" "$(cat <<'EOB'
## Problem

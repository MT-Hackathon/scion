...
# ///
"""
State GitLab API shared module for procurement-web workflow scripts.

Targets git.mt.gov (Montana state GitLab instance).
Uses STATE_GITLAB_PERSONAL_ACCESS_TOKEN from .env via python-dotenv.
"""
...
PROJECT_PATH = "Enterprise/DOA/SITSD/ATSB/software-factory/presentation/doa/procurement-workflow/procurement-web"
PROJECT_PATH_ENCODED = urllib.parse.quote(PROJECT_PATH, safe="")
API_BASE = "https://git.mt.gov/api/v4"
TOKEN_ENV_VAR = "STATE_GITLAB_PERSONAL_ACCESS_TOKEN"
...
def require_token() -> str:
    """Return state GitLab token from environment or exit with error."""
...
def build_headers(token: str) -> dict[str, str]:
    ...
    return {
        ...
        "User-Agent": "procurement-web-state-gitlab-scripts",
    }
...
def project_api_url() -> str:
    """Return the base API URL for the project."""
    return f"{API_BASE}/projects/{PROJECT_PATH_ENCODED}"


def issues_api_url() -> str:
    """Return the issues API URL for the project."""
    return f"{project_api_url()}/issues"


def merge_requests_api_url() -> str:
    """Return the merge requests API URL for the project."""
    return f"{project_api_url()}/merge_requests"


def pipelines_api_url() -> str:
    """Return the pipelines API URL for the project."""
    return f"{project_api_url()}/pipelines"


def labels_api_url() -> str:
    """Return the labels API URL for the project."""
    return f"{project_api_url()}/labels"


def milestones_api_url() -> str:
    """Return the milestones API URL for the project."""
    return f"{project_api_url()}/milestones"
...
def perform_request(
    ...
    try:
        ...
        sys.stderr.write(f"State GitLab API error {error.code}: {detail}\n")

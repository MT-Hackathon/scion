...
- **Mock-Boundary Mandate**: Mock at the transport or I/O layer you are testing, not above it. Function-level mocks (mocking `getMcpStatus()`) test caller behavior, not function correctness — the function's code never executes. For IPC wrappers, mock at `invoke`; for HTTP clients, mock at `fetch`. Coverage dark zones form at mock ceilings: if every test mocks a function, that function's contract is never tested by any test in the suite.
- **Shadow Implementation Anti-Pattern**: Test helper functions that duplicate production logic are invisible to formula drift — if the production code changes, the shadow tests still pass. Before writing a test helper that re-implements a formula, ask: can I make the production function `pub(crate)` and call it directly? If the formula is embedded in a closure or ORM mapping, extract it to a named function so tests reach production code. Shadow implementations have value as mathematical specification documents, but they don't substitute for tests of the real thing.
- **Null Test Anti-Pattern**: A test that can only pass (checking `is_finite()`, `result.len() == 2`, or asserting a hardcoded constant like `0.3 < 0.6`) provides false confidence — it occupies space in the test suite without guarding any behavior. Every test must have a realistic failure mode. If you can't construct an input that makes the test fail, the test is documentation, not verification.

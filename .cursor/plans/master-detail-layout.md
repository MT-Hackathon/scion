# Plan: MasterDetail Layout Refactor

**Slug**: `master-detail-layout` (filename: `.cursor/plans/master-detail-layout.md`)  
**Triggered by**: Stability walk — drawer sandwiching between header/footer, theme cascade revealing fragmented component architecture  
**Status**: Draft
...
| 3 | QA | Full pass: scroll containment, z-index audit, responsive breakpoints, theme switch, all tests | All acceptance criteria verified | `npm test -- --run && npx svelte-check` |
...
**Scope**: All acceptance criteria R1-R9.
...
**Related work**:
- Telemetry instrumentation (Rust `tracing` crate, frontend wrapper) — discussed but not scoped here. Future issue.
